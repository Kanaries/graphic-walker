...
export class VizSpecStore {
    ...
    get paintInfo() {
        ...
        if (existPaintField) {
            const param: IPaintMap | undefined = existPaintField.expression?.params.find((x) => x.type === 'map')?.value;
            if (param) {
                return {
                    type: 'exist',
            ...
            const paramV2: IPaintMapV2 | undefined = existPaintField.expression?.params.find((x) => x.type === 'newmap')?.value;
            if (paramV2) {
                return {
                    type: 'exist',
                    item: paramV2,
