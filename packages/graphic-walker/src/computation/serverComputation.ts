...
const defaultServer = 'https://kanaries.net';
...
const makeComputationService = (config: IServerComputationOptions): IComputationFunction => {
    ...
    const server = ('server' in config ? config.server?.replace(/\/$/, '') : undefined) ?? defaultServer;
    return async function httpComputationService<R extends IRow[]>(data: IDataQueryPayload): Promise<R> {
        const res = await fetch(`${server}/api/ce/dataset/v2/query`, {
            method: 'POST',
            credentials: 'include',
            headers: {
