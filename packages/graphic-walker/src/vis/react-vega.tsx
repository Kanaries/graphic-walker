...
const ReactVega = forwardRef<IReactVegaHandler, ReactVegaProps>(function ReactVega(props, ref) {
    ...
    const allFieldIds = useMemo(
        () => [...rows, ...columns, color, opacity, size].filter((f) => Boolean(f)).map((f) => (f as IViewField).fid),
        [rows, columns, color, opacity, size]
    );
    ...
    useEffect(() => {
        ...
        const rowFacetField =
            rowLeftFacetFields.length > 0 ? rowLeftFacetFields[rowLeftFacetFields.length - 1] : NULL_FIELD;
        const colFacetField =
            colLeftFacetFields.length > 0 ? colLeftFacetFields[colLeftFacetFields.length - 1] : NULL_FIELD;
        ...
        if (rowRepeatFields.length <= 1 && colRepeatFields.length <= 1) {
            ...
            if (viewPlaceholders.length > 0 && viewPlaceholders[0].current) {
                embed(viewPlaceholders[0].current, spec, {
                    mode: 'vega-lite',
                    actions: showActions,
                    timeFormatLocale: getVegaTimeFormatRules(i18n.language),
                    config: vegaConfig,
                }).then((res) => {
            ...
            const throttledParamStore$ = combinedParamStore$.pipe(
                op.throttleTime((dataSource.length / 64) * rowRepeatFields.length * colRepeatFields.length, undefined, {
                    leading: false,
                    trailing: true,
                })
            );
            ...
            for (let i = 0; i < rowRepeatFields.length; i++) {
                for (let j = 0; j < colRepeatFields.length; j++, index++) {
                    ...
                    const node =
                        i * colRepeatFields.length + j < viewPlaceholders.length
                            ? viewPlaceholders[i * colRepeatFields.length + j].current
                            : null;
                    let commonSpec = { ...spec };
                    ...
                    if (node) {
                        embed(node, ans, {
                            ...
                            const paramStores = (res.vgSpec.data?.map((d) => d.name) ?? [])
                                .filter((name) =>
                                    [BRUSH_SIGNAL_NAME, POINT_SIGNAL_NAME].map((p) => `${p}_store`).includes(name)
                                )
    ...
    }, [dataSource, allFieldIds, rows, columns, defaultAggregate, geomType, color, opacity, size, shape, theta, radius, viewPlaceholders, rowFacetFields, colFacetFields, rowRepeatFields, colRepeatFields, stack, showActions, interactiveScale, layoutMode, width, height, vegaConfig, details, text]);
    ...
    return (
        <CanvaContainer rowSize={Math.max(rowRepeatFields.length, 1)} colSize={Math.max(colRepeatFields.length, 1)}>
