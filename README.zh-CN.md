...
+ 可手写的图表描述文法（TerseSpec）：按字段名 + `agg(field)` 简写描述图表，经 `normalize()` 展开为完整图表。🆕
...
### TerseSpec：可手写的图表描述文法 🆕

Graphic Walker 内部存储图表使用的是一种冗长的、面向机器的规范格式（`IChart`）：字段带生成的 id，每个通道、字段池、布局项都是显式的。**TerseSpec** 是叠在它上面的一层可手写文法——按字段*名*引用，由 `normalize()` 展开为任何组件都能接受的完整图表：
...
| 语法 | 含义 |
| --- | --- |
| `"x": "Manufacturer"` | 按字段名引用 |
| `"y": "mean(Price_in_thousands)"` | 聚合简写：`sum` / `mean` / `count` / `median` / `min` / `max` / `variance` / `stdev` |
| `"x": "fid:col_0_64"` | `fid:` 前缀绕过按名解析 |
| `{ "field", "aggregate", "sort", "timeUnit" }` | 通道级选项的对象形式；`timeUnit` 是真正的查询级日期钻取 |
| `"computed": [{ "name", "expr" \| "bin" \| "log" }]` | 内联计算字段；`expr` 里的字段引用需加引号，如 `'"Horsepower" / "Curb_weight"'` |
| `"filters": [{ "field", "oneOf" \| "notIn" \| "range" \| "timeRange" }]` | 行过滤；`timeRange` 只接受 epoch 毫秒 |
| `"aggregate": false` / `"stack"` / `"limit"` / `"sort"` | 图表级开关 |
| `"config"` / `"layout"` | canonical 直通逃生舱，最后合并、优先级最高 |

`normalize(input, fields)` 是统一入口：除 TerseSpec 外也接受完整图表、部分图表、旧版持久化格式与 Vega-Lite 风格输入，始终返回带 `$schema` 的完整 `IChart`，并顺带校验计算字段引用——未知字段名会快速报错并附近似候选。

TerseSpec 只用于书写。持久化与导入导出仍使用 canonical 格式，已保存的图表不受任何影响。

👉 在 [playground](https://graphic-walker.kanaries.net/examples/TerseSpec) 里试玩交互式文法示例；完整契约见 [docs/terse-spec-design.md](./docs/terse-spec-design.md)；JSON Schema：`https://graphic-walker.kanaries.net/tersespec_v1.json`。
